...
{
    static isPrimitive(obj: StringConstructor | NumberConstructor | BooleanConstructor)
    {
        switch (typeof obj)
        {
            case "string":
    ...
    static deserialize<T>({ clazz, jsonObject }: { clazz: { new(): T; }; jsonObject: { [x: string]: any; }; })
    {
        ...
        Object.keys(obj).forEach((key) =>
        {
            let propertyMetadataFn: (IJsonMetaData) => any = (propertyMetadata) =>
            {
                    ...
                    if (metadata.clazz || MapUtils.isPrimitive(clazz))
                    {
                        if (innerJson && MapUtils.isArray(innerJson))
                        {
                            return innerJson.map(
                                (item) => MapUtils.deserialize({ clazz: metadata.clazz, jsonObject: item })
                            );
                        } else
                        {
                            return undefined;
                ...
                {
                    return MapUtils.deserialize({ clazz, jsonObject: innerJson });
                } else
                {
